...
export class Histogram {
  ...
  makePalette(pal0: Readonly<Palette>) {
    ...
    if (boxList.length < this.pal.length) {
      // Однако если в палитре есть заданные цвета, то их удалять нельзя.
      let newLen = this.pal.length;
      while (newLen > boxList.length && isFreeColor(this.pal[newLen - 1]!))
        --newLen;
      this.pal = this.pal.slice(0, newLen);
    }
